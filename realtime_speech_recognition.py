            ...
            if event == 'A':  # 最終認識結果
                try:
                    ...
                    if 'text' in result:
                        ...
                        print(f"認識結果を追加: {result['text']}")  # デバッグ用
                    else:
                        print(f"textキーが見つかりません: {result}")  # デバッグ用
                except json.JSONDecodeError as e:
                    print(f"JSONデコードエラー: {e}")  # デバッグ用
            elif event == 'e':
                print("セッション終了イベントを受信")  # デバッグ用
                self.set_ws_connected(False)
                ws.close()
            else:
                print(f"未処理のイベント: {event}")  # デバッグ用
        except Exception as e:
            print(f"メッセージ処理エラー: {e}")  # デバッグ用
